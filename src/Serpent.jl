...
export serpentencrypt, serpentdecrypt
    ...
    function getbit(x, i)
        return Int64(x[floor(beg + i / 32)] >>> (i % 32)) & Int64(0x01)
    end
    ...
    function setbit(x, i, v)
        if ((v & 0x01) == 1)
            x[beg + convert(Int64, floor((i) / 32))] |= Int64(1) << (i % 32) 
        else
            x[beg + convert(Int64, floor( (i) / 32))] &= ~(Int64(1) << (i % 32)) 
        end
        return x
    ...
    function serpentencrypt(block::Array{UInt8}, key::Array{UInt8})
        k = makekey(key)
        return blockencrypt(block, 1, k)
    end

    function serpentdecrypt(block::Array{UInt8}, key::Array{UInt8})
        k = makekey(key)
        return blockdecrypt(block, 1, k)
    end
end
